...
struct KakaoMapOptions {
    ...
    var overlay: String?
    var language: String
    var defaultPosition: MapPoint = .init(longitude: 127.108678, latitude: 37.402001)
    var defaultLevel: Int = 17
    var enabled: Bool = true
    var buildingScale: Float = 1.0
    var padding: UIEdgeInsets = .init(top: 0, left: 0, bottom: 0, right: 0)
    var logoPosition: KakaoMapPosition = .init(alignment: .bottomRight, x: 0, y: 0)
    var poiOptions: KakaoMapPoiOptions = .init(clickable: true, enabled: true, scale: PoiScaleType.regular)
    var compassOptions: CompassOptions = .init(enabled: false, position: KakaoMapPosition(alignment: .bottomRight, x: 0, y: 0))
    var scaleBarOptions: ScaleBarOptions = .init(enabled: false, position: KakaoMapPosition(alignment: .bottomRight, x: 0, y: 0), autoDisabled: true, fadeInOutOptions: FadeInOutOptions(fadeInTime: 300, fadeOutTime: 300, retentionTime: 3000))
}
...
extension NSDictionary {
    ...
    func toLodLabelLayerOptions() -> LodLabelLayerOptions {
        return LodLabelLayerOptions(layerID: self["layerID"] as! String, competitionType: CompetitionType(rawValue: (self["competitionType"] as! Int))!, competitionUnit: CompetitionUnit(rawValue: (self["competitionUnit"] as! Int))!, orderType: OrderingType(rawValue: (self["orderType"] as! Int))!, zOrder: self["zOrder"] as! Int, radius: 0x10)
    }

    func toRouteLineOptions() -> PerLevelRouteStyle {
        let lineWidth = self["lineWidth"] as? Int ?? 0
        let lineColor = self["lineColor"] as? Int ?? 0

        return PerLevelRouteStyle(width: UInt(lineWidth * 2), color: UIColor(hex: lineColor), level: 0)
    }
}
